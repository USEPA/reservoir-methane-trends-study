...
%% DESPIKING
 ...
 hh=find(~isnan(wc(use)));% Find the non-nan values

 if length(hh)>0.1*nw
 a=polyfit(use(hh),wc(use(hh)),1);%find a linear trend in the length of the dataset (e.g. seasonal trend)
 wcs=wc(use)-polyval(a,use);% remove the seasonal trend from dataset
% figure;plot(wc(use));hold on;plot(polyval(a,use));
% title(['linear trend in window ' num2str(y) ' out of ' num2str(R) ' realizations'])
 ...
 bad=find(abs(red(dwcs1))>tr | abs(red(dwcs2))>tr |abs(wcsC)>tr | wc(use)>max | wc(use)<min);
     nbad=length(bad);
