...
source("C:/R_Projects/R-User-Group/contributedCode/waterTempTimeSeries/filledcontour3.R")
source("C:/R_Projects/R-User-Group/contributedCode/waterTempTimeSeries/filledlegend.R")

# create new graphic with two contour plots
tiff("C:/R_Projects/actonFluxProject/figures/test.tif", res=1200, compression="lzw", 
...
par(new = "TRUE",              
    plt = c(0.18,0.8,0.55,0.90),  # using plt instead of mfcol (compare
...
wtr <- filter(c.RBR, datetime>"2017-06-01")
...
filled.legend(
  ...
  xlim = range(wtr.dates),
  ylim = c(max(depths), 0),
  plot.title = title(main = "Celsius", line = .5)
)
