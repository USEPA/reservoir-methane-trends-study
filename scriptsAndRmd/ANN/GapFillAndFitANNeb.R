...
############# ANN Fitting #############
#######################################
plotGaps(fluxDatEb, "ch4_flux")
sum(is.na(fluxDatEb$ch4_flux)) / nrow(fluxDatEb) # 75% missing!
## Data prep
annCols <- c("ch4_flux","FilledSedT",  "FilledAirT", "FilledWindSpeed", "fuzzyRAD",
             "FilledStaticPress", "FilledStaticPressChg", "ebCh4_deepGf", "ebCh4_shalGf")
             # , "ebCh4_shallow",
             # "ebCh4_deep")
annDat <- fluxDatEb[,annCols]
...
fitModels <- TRUE      #change this from FALSE to TRUE to run the model (??)
if(fitModels){
  ...
  save(simList, file = "output/annSimulationListEb.RData")
}
...
summary(r2Sims)
sum(r2Sims>= 0.3)/length(r2Sims) # 20% are higher than 0.5.
...
save(bestANNs, file = "output/BestANNsEb.RData")
...
d <- data.frame("Flux" = validFlux,
                "Preds" = validMedians)
lms <- c(-0.5,1.5)
p <- ggplot(d, aes(x = Flux, y = Preds)) + geom_point(alpha=0.3) + 
...
ch4ANN <- ifelse(is.na(fluxDatEb$ch4_flux), 
                 ch4ANNFits,
                 fluxDatEb$ch4_flux)
fluxDatEb$ch4_preds <- ch4ANN
write.csv(fluxDatEb, "output/fluxDatEbaWithFits.csv", row.names = FALSE)

ggplot(fluxDatEb, aes(datetime, ch4_preds))+
  geom_line(alpha=0.5)+
  geom_point(data=fluxDatEb, aes(datetime, ch4_flux), color="red", alpha=0.1)+
  ylim(-1, 2)
ggplot(fluxDatEb, aes(datetime, ch4_flux*60*60*16/1000))+
  geom_point(alpha=0.1)+
  ylim(-25, 100)
#linear regression: EC flux vs. shallow bubble trap
lmShalTrap<-lm(ch4_flux ~ ebCh4_shalGf, data = fluxDatEb)
summary(lmShalTrap)  #R2 = 0.01
ggplot(fluxDatEb, aes(ebCh4_shalGf, ch4_flux*60*60*16/1000))+
  geom_point(alpha=0.1)+
  stat_smooth(method="lm")
#linear regression: EC flux vs. deep bubble trap
lmDeepTrap<-lm(ch4_flux ~ ebCh4_deepGf, data = fluxDatEb)
summary(lmDeepTrap)   #R2 = 0.04
ggplot(fluxDatEb, aes(ebCh4_deepGf, ch4_flux*60*60*16/1000))+
  geom_point(alpha=0.1)+
  stat_smooth(method="lm")

fluxDatEb$ebCh4_mean<-(fluxDatEb$ebCh4_deepGf+fluxDatEb$ebCh4_shalGf)/2
#linear regression: EC flux vs. avg bubble trap
lmAvgTrap<-lm(ch4_flux ~ ebCh4_mean, data = fluxDatEb)
summary(lmAvgTrap)   #R2 = 0.04
