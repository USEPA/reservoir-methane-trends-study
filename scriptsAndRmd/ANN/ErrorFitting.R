...
fitANN <- function(trn){
  ...
  tmpTest <- scaledDat[testInds,]
  #tmpTest = scaledDat[-trn,] #need to make this half of whats left
  tmpValid = scaledDat[-c(trn, tmpTest),]
  tmpTrain <- subset(tmpTrain, !is.na(ch4_flux))
  tmpTest <- subset(tmpTest, !is.na(ch4_flux))
  tmpValid <- subset(tmpValid, !is.na(ch4_flux))
