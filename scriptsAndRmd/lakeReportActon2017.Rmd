...
actonSitesPlot07_84 <- spTransform(x = actonSitesPlot07, #reproject
                            CRS("+proj=longlat +datum=WGS84")) # projection for google maps

actonSitesPlot07_84@data <- mutate(actonSitesPlot07_84@data, 
                            long=coordinates(actonSitesPlot07_84)[,1], # add long to @data slot
                            lat=coordinates(actonSitesPlot07_84)[,2]) # add lat to @data slot

actonSitesPlot08 <- readOGR(dsn = globalWd,
                            layer = "acton08SitesEqAreaDataEC") # shapefile with data for Aug
actonSitesPlot08_84 <- spTransform(x = actonSitesPlot08, #reproject
                            CRS("+proj=longlat +datum=WGS84")) # projection for google maps

actonSitesPlot08_84@data <- mutate(actonSitesPlot08_84@data, 
                            long=coordinates(actonSitesPlot08_84)[,1], # add long to @data slot
                            lat=coordinates(actonSitesPlot08_84)[,2]) # add lat to @data slot

actonSitesPlot10 <- readOGR(dsn = globalWd,
                            layer = "acton10SitesEqAreaDataEC") # shapefile with data for Oct
actonSitesPlot10_84 <- spTransform(x = actonSitesPlot10, #reproject
                            CRS("+proj=longlat +datum=WGS84")) # projection for google maps

actonSitesPlot10_84@data <- mutate(actonSitesPlot10_84@data, 
                            long=coordinates(actonSitesPlot10_84)[,1], # add long to @data slot
                            lat=coordinates(actonSitesPlot10_84)[,2]) # add lat to @data slot
...
with(actonSitesPlot07_84@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 scale.y=0.5,                                   # scale y axis (increase by 175% (1.75))
                                   main=expression(July~10^{th}~Ebullitive~Methane~Emissions),      # main plot title
...
with(actonSitesPlot08_84@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 scale.y=0.5,                                   # scale y axis (increase by 175% (1.75))
                                   main=expression(August~31^{st}~Ebullitive~Methane~Emissions),      # main plot title
                                   xlab="Latitude",                                       # x-axis label 
                                   ylab="",                                       # y-axis label
                                 y.margin.add=0.5, 
                                 #x.margin.add=0.5,
...
actonSitesPlot10_84@data <- merge(actonSitesPlot10_84@data, # add emission to point shp
                             filter(eqAreaData, Lake_Name == "Acton Lake Oct") %>%
                               select(ch4.erate.mg.h, siteID))
...
with(actonSitesPlot10_84@data, {
  ch4_plot <- scatterplot3d_edit(x=coords$long, y=coords$lat, z=rep(0,length(coords$long)),   # x y and z data, respectively
                                 ...
                                 scale.y=0.5,                                   # scale y axis (increase by 175% (1.75))
                                   main=expression(October~4^{th}~Ebullitive~Methane~Emissions),      # main plot title
                                   xlab="Latitude",                                       # x-axis label 
                                   ylab="",                                       # y-axis label
                                 y.margin.add=0.5, 
                                 #x.margin.add=0.5,
