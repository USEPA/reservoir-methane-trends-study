...
plotGaps(filter(fluxDat, datetime>"2018-05-06 00:00:00"), "ustar")
...
fluxDatToUse<-subset(fluxDat, fluxDat$datetime>"2018-05-07 00:00:00")
plotGaps(fluxDatToUse, "ch4_flux")
sum(is.na(fluxDatToUse$ch4_flux)) / nrow(fluxDatToUse) #28% missing
...
if(fitModels){
  ...
  save(simList, file = "output/annSimulationListAq2018.RData")
}
...
save(bestANNs, file = "output/BestANNsAq2018.RData")
...
write.csv(fluxDatToUse, "output/FluxDataWithFitsAq2018.csv", row.names = FALSE)
