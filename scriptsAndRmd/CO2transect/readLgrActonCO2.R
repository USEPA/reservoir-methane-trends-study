# SCRIPT TO LOAD AND PROCESS LGR FILES FROM 2018 CO2 SURVEY AT ACTON LAKE.
...
txtFiles <- list.files("L:/Priv/Cin/NRMRL/ReservoirEbullitionStudy/ebullition2018/CO2survey/2018-08-03", 
...
#########not needed for CO2 survey files#######----
...
for (i in 1:length(txtFiles)) {  # loop to read and format each file
  gga.i <- read.table(paste("L:/Priv/Cin/NRMRL/ReservoirEbullitionStudy/ebullition2018/CO2survey/2018-08-03/", 
                            txtFiles[i], sep=""),
                      sep=",",  # comma separate
