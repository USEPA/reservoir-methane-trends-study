...
epFiles <- list.files("C:/R_Projects/EC/eddyproout/fullOutput/continuous", 
...
for (i in 1:length(epFiles)) {  # loop to read and format each file
  ep.i <- read.table(paste("C:/R_Projects/EC/eddyproout/fullOutput/continuous/", 
                           epFiles[i], sep=""),
                     sep=",",  # comma separate
  ...
  ep.i <- select(ep.i, RDateTime, H, LE, qc_H, qc_LE, wind_dir, ustar, zL, w_var, w_unrot, w_rot, wind_speed, max_wind_speed)  # select minimal columns of interest
...
wPlotWD<-ggplot(epOut, aes(RDateTime, w_unrot))+
  geom_point()
  
wPlotWD

wPlotWD+facet_grid(epOut$wdfact4~.)

epOut$timeFact<-cut(epOut$RDateTime, breaks = c(as.POSIXct("2016-10-01 12:30:00", tz = "UTC"),  #deployment date, on tripod, height = 2.4 m 
                                                as.POSIXct("2017-01-13 16:00:00", tz = "UTC"),  #redeployed on pylong, height = 2.4 m 
                                                as.POSIXct("2017-04-04 12:00:00", tz = "UTC"),  #dock installed. height above water is 2.5 m 
                                                as.POSIXct("2017-04-19 16:00:00", tz = "UTC"),  #elevated height to 2.83 m 
                                                as.POSIXct("2017-05-19 20:00:00", tz = "UTC")), #end of time period
                    labels = c("tripod 2.4m", "pylon pre-dock 2.5m", "pylon post-dock 2.5m", "pylon post-dock 2.8m")) 

summary(epOut$timeFact)

wPlotWD+facet_grid(epOut$wdfact4 ~ epOut$timeFact)
...
wPlotWD<-ggplot(epOut, aes(RDateTime, w_unrot))+
  ...
  #geom_errorbar+
  facet_grid(epOut$wdfact4~epOut$timeFact, scales = "free_x", 
