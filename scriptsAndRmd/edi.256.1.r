...
dt3_2018<-read.xls(paste(myWd, "/vanniEDI/discharge2018.xlsx", sep=""), sheet=2)
...
chl2017<-read.xls(paste(myWd, "/vanniEDI/Chl2017.xlsx", sep=""), sheet=3)
chl2017$RDate<-as.Date(chl2017$Date, format="%Y-%m-%d")

chl2018<-read.xls(paste(myWd, "/vanniEDI/Chl2018.xlsx", sep=""), sheet=3)
...
ggplot(filter(metaDataSonde, Sample.depth.m<2, Site=="u14"| Site=="u12", sample.date<"2018-01-01"),
       aes(sample.date, DO.mgL))+
  annotate("rect", xmin=as.Date("2017-05-24"),
           xmax=as.Date("2017-06-04"),
           ymin=-Inf, ymax=Inf, alpha=0.5)+
  # annotate("rect", xmin=as.Date("2018-05-24"),
  ...
  geom_point(data=filter(dt1, Date>"2017-01-01", Date<"2018-01-01", Depth<2), 
             aes(Date, DissolvedOxygen), alpha=0.2)

ggplot(filter(metaDataSonde, Sample.depth.m<2, Site=="u12", monthday>"2019-04-01"),
       aes(monthday, DO.percent))+
  annotate("rect", xmin=as.Date("2019-05-24"),
           xmax=as.Date("2019-06-04"),
           ymin=-Inf, ymax=Inf, alpha=0.5)+
  # annotate("rect", xmin=as.Date("2018-05-24"),
  #          xmax=as.Date("2018-06-04"),
  #          ymin=-Inf, ymax=Inf, alpha=0.5)+
  geom_point(color="blue")+#as.factor(Site)))+
  #geom_line(aes(color=as.factor(Site)))+
  # geom_point(data=filter(dt1, Date>"2017-01-01", Depth<2), 
  #            aes(Date, DissolvedOxygen), alpha=0.2)+
  facet_grid(year~.)

ggplot(filter(metaDataSonde, Site=="u14"), aes(sample.date, Turbidity.ntu))+
  annotate("rect", xmin=as.Date("2017-05-24"),
           xmax=as.Date("2017-06-04"),
  ...
  scale_x_date(labels=date_format("%b %Y", tz="UTC"),
               breaks=date_breaks("2 month"),
               limits = c(as.Date("2017-04-01"), as.Date("2018-11-20")))
