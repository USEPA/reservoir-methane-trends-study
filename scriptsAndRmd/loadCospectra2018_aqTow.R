...
csvFilesAll<-list.files(csvDir, recursive=TRUE)
...
cospectra2018List <- list()  # Empty list to hold results
...
for (i in 1:length(csvFiles2018)) {  # loop to read and format each file
  cospectra2018.i <- read.table(paste(csvDir, 
                            csvFiles2018[i], sep="/"),
                      sep=",",  # comma separate
 ...
 #SELECT WHICH FIELDS OF THE COSPECTRA FILE WE WANT TO KEEP
   cospectra2018.i <- select(cospectra2018.i, naturalFreq, normFreq, uSpec, vSpec, TsSpec, wSpec, co2Spec, h2oSpec, ch4Spec,
...
dateTimeList <- substr(csvFiles2018DF$csvFiles2018, start=1, stop=13) #cha-ching!
#if there are sub-folders:
dateTimeList <- substr(csvFiles2018DF$csvFiles2018, start=15, stop=27)
...
cospectraDates<-cbind(cospectra2018, RDateTime)
 ...
 summary(cospectraDates$RDateTime) 
...
######Aquatic Tower
###NEAR NEUTRAL
#Winds from S, near neutral, aquatic tower
Lake_S_Neut<-filter(cospectraEC, zL>-0.2, zL<0.1,
               RDateTime>"2018-08-01",
                     wind_dir>150, wind_dir<200,
                     wSpec!= -9999)
#Winds from NW, near neutral
Lake_NW_Neut<-filter(cospectraEC, zL>-0.2, zL<0.1,
                RDateTime>"2018-08-01",
                wind_dir>250, wind_dir<360,
                      wSpec!= -9999)

####STABLE
#Winds from S,  stable
Lake_S_Stab<-filter(cospectraEC, zL>0.1,
               RDateTime>"2018-08-01",
                  wind_dir>150, wind_dir<200,
                     wSpec!= -9999)
#Winds from NW, stable
Lake_NW_Stab<-filter(cospectraEC, zL>0.1,
                     RDateTime>"2018-08-01",
                wind_dir>250, wind_dir<360,
                         wSpec!= -9999)

####UNSTABLE (not defined in Novick, but I'll define as -2 to -0.2)---------
#winds from S unstable, 
Lake_S_Unstab<-filter(cospectraEC, zL<-0.2, 
                 RDateTime>"2018-08-01",
                 wind_dir>150, wind_dir<200,
                     wSpec!= -9999)
#Winds from NW, unstable
Lake_NW_Unstab<-filter(cospectraEC, zL>-2, zL<-0.2,
                       RDateTime>"2018-08-01",
                    wind_dir>250, wind_dir<360,
                         wSpec!= -9999)


####Dock: E (50 - 125, ejection effect) vs S (150 - 195)
###NEAR NEUTRAL
#Winds from S, near neutral, aquatic tower
Dock_E_Neut<-filter(cospectraEC, zL>-0.2, zL<0.1,
                    RDateTime<"2018-08-01",
                    wind_dir>50, wind_dir<125,
                    wSpec!= -9999)
#Winds from NW, near neutral
Dock_S_Neut<-filter(cospectraEC, zL>-0.2, zL<0.1,
                     RDateTime<"2018-08-01",
                     wind_dir>150, wind_dir<195,
                     wSpec!= -9999)

####STABLE
#Winds from S,  stable
Dock_E_Stab<-filter(cospectraEC, zL>0.1,
                    RDateTime<"2018-07-01",
                    wind_dir>50, wind_dir<125,
                    wSpec!= -9999)
#Winds from NW, stable
Dock_S_Stab<-filter(cospectraEC, zL>0.1,
                     RDateTime<"2018-08-01",
                     wind_dir>150, wind_dir<195,
                     wSpec!= -9999)
...
####UNSTABLE (not defined in Novick, but I'll define as -2 to -0.2)---------
#winds from S unstable, 
Dock_E_Unstab<-filter(cospectraEC, zL<-0.2, 
                      RDateTime<"2018-08-01",
                      wind_dir>50, wind_dir<125,
                      wSpec!= -9999)
#Winds from NW, unstable
Dock_S_Unstab<-filter(cospectraEC, zL>-2, zL<-0.2,
                       RDateTime<"2018-08-01",
                       wind_dir>150, wind_dir<195,
                       wSpec!= -9999)
...
## For Ameriflux pi meeting, want to compare S measurements (unrotated w ~0.2) and NW

S

#### Establish discrete freqency bins, discard the extreme 10%, ensemble averaging the spectral values 
listOfDataFrames <- list(Lake_S_Neut, Lake_NW_Neut, Lake_S_Stab, Lake_NW_Stab, Lake_S_Unstab, Lake_NW_Unstab,
                         Dock_S_Neut, Dock_E_Neut, Dock_S_Stab, Dock_E_Stab, Dock_S_Unstab, Dock_E_Unstab)
                         
names(listOfDataFrames) = c("Lake South Neutral", "Lake NW Neutral", "Lake South Stable", "Lake NW Stable", 
                            "Lake South Unstable", "Lake NW Unstable", "Dock S Neutral", "Dock E Neutral",
                            "Dock S Stable", "Dock E Stable", "Dock S Unstable", "Dock E Unstable")
neutStabList<-c("neutral", "neutral", "stable", "stable", "unstable","unstable",
                "neutral", "neutral", "stable", "stable", "unstable","unstable")
setupList<-c("Lake w>0", "Lake w=0", "Lake w>0", "Lake w=0", "Lake w>0", "Lake w=0", "Dock w=0", "Dock w>0", 
             "Dock w=0", "Dock w>0","Dock w=0", "Dock w>0")
...
# w'Ts' cospectra
ggplot(filter(SpectraByFreqBinAdv, stabClass=="neutral"), aes(binnedFreq,  binnedWTs))+
  geom_point(aes(color=towerSetup, shape=stabClass),  alpha=0.7)+
  scale_fill_manual(values=c("red", "pink", "navy", "skyblue"))+
...
# w'CH4 cospectra
ggplot(filter(SpectraByFreqBinAdv, stabClass == "neutral"), aes(binnedFreq,  binnedWCh4))+
  geom_point(aes(color=towerSetup, shape=stabClass), alpha=0.7)+
  #geom_line(aes(color=towerSetup))+
  geom_abline(linetype=2, slope=-(5/3), intercept = 10^-3)+
  geom_abline(linetype=3, slope=-(10/3), intercept = 10^-2)+
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)),
                limits = c(10^-3, 10^0))+
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
