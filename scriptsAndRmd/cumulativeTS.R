...
DailyANNFluxes<-epDataFilled %>%
  group_by(date) %>%
  dplyr::summarize(meanCH4Flux = (mean(ch4_preds6.01, na.rm=TRUE))/1000*16*60*60)
...
DailyShalFluxes$Label<-"b) Shallow AFT"
...
DailyDeepFluxes$Label<-"c) Deep AFT"

DailyANNF3<-select(DailyANNFluxes, date, meanCH4Flux, Label)

dailyF3list<-list()
dailyF3list[[1]]<-DailyANNF3
dailyF3list[[2]]<-DailyShalFluxes
dailyF3list[[3]]<-DailyDeepFluxes

dailyFluxF3<-do.call("rbind", dailyF3list)
...
Figure3<-ggplot(flux30mintsF3, aes(datetime, ch4_flux_filled.f3))+
  ...
  geom_line(data=dailyFluxF3, aes(date, meanCH4Flux), color="red", size=1)+
  facet_grid(Label~.)+
